...
public:
	...
	explicit constexpr Vector2(int inX, float inY)
	: x(static_cast<float>(inX))
	, y(inY)
	{
	}

	explicit constexpr Vector2(float inX, int inY)
	: x(inX)
	, y(static_cast<float>(inY))
	{
	}
	...
	// Vector Comparation
	[[nodiscard]] friend bool operator==(const Vector2& a, const Vector2& b)
	{
